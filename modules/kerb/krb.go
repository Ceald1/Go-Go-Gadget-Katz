...
func TGT(domain, username, password, outputFile string) (encodedTicket string, err error) {
	...
	if err != nil {
		err = fmt.Errorf("error converting domain to UTF16: %v", err)
		return
	}
	
	userPtr, err := syscall.UTF16PtrFromString(username)
	if err != nil {
		err = fmt.Errorf("error converting username to UTF16: %v", err)
		return
	}
	
	passPtr, err := syscall.UTF16PtrFromString(password)
	if err != nil {
		err= fmt.Errorf("error converting password to UTF16: %v", err)
		return
	}
	...
	if !success {
		err= fmt.Errorf("failed to acquire credentials, last error: 0x%x", lastError)
		return
	}
	...
	if err != nil {
		err= fmt.Errorf("error converting target name to UTF16: %v", err)
		return
	}
	...
	if status != SEC_E_OK && status != SEC_I_CONTINUE_NEEDED {
		winErr, _, _ := procGetLastError.Call()
		err= fmt.Errorf("InitializeSecurityContext failed: status=0x%x, winErr=0x%x, err=%v", 
	...
	if outBuf.pvBuffer != 0 && outBuf.cbBuffer > 0 {
		...
		encodedTicket = base64.StdEncoding.EncodeToString(tickData)
		if err != nil {
			err= fmt.Errorf("failed to write Kirbi ticket to file: %v", err)
			return
		}
